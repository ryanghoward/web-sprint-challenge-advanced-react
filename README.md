...
componentWillMount is executed before rendering, on both the server and the client side, componentWillUpdate is called just before rendering. 
...
Arrange: This is the point at which we tell the test where we're looking.
Act: This is where we define the elements that will be targeted and how we will interact with them. 
Assert: Now we tell the test what we're looking for in terms of a result.
